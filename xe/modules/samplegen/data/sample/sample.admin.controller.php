<?php
class {$modulename}AdminController extends {$modulename} {
	function init() {
	}

	function proc{$Modulename}AdminSetup() {
		...
		$module_info = $o{$Modulename}Model->getModulesInfo();
		...
		// module_config에 입력
		${$modulename}->mid = $args->mid;
		$output = $oModuleController->insertModuleConfig('{$modulename}', ${$modulename});

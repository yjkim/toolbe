<?php
class samplegenAdminController extends samplegen {
	function init() {
	}

	function procSamplegenAdminSetup() {
		...
		// 기본설정가져오기
		$oSamplegenModel = &getModel('samplegen');
		$module_info = $oSamplegenModel->getModulesInfo();
		...
		// module_config에 입력
		$samplegen->mid = $args->mid;
		$output = $oModuleController->insertModuleConfig('samplegen', $samplegen);

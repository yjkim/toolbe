<?php
class jsonformatterAdminController extends jsonformatter {
	function init() {
	}

	function procJsonformatterAdminSetup() {
		...
		// 기본설정가져오기
		$oJsonformatterModel = &getModel('jsonformatter');
		$module_info = $oJsonformatterModel->getModulesInfo();
		...
		// module_config에 입력
		$jsonformatter->mid = $args->mid;
		$output = $oModuleController->insertModuleConfig('jsonformatter', $jsonformatter);

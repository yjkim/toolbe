<?php
class myipAdminController extends myip {
	function init() {
	}

	function procMyipAdminSetup() {
		...
		// 기본설정가져오기
		$oMyipModel = &getModel('myip');
		$module_info = $oMyipModel->getModulesInfo();
		...
		// module_config에 입력
		$myip->mid = $args->mid;
		$output = $oModuleController->insertModuleConfig('myip', $myip);
